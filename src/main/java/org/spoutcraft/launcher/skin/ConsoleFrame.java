...
public class ConsoleFrame extends JFrame implements MouseListener {
	 ...
	 * 
	 * @param numLines
	 *            number of lines to show at a time
	 * @param colorEnabled
	 *            true to enable a colored console
	 */
	 ...
	 *            process to track
	 * @param killProcess
	 *            true to kill the process on console close
	 */
	public ConsoleFrame(int numLines, boolean colorEnabled,
	 ...
	 *            line
	 * @param attributes
	 *            attribute set, or null for none
	 */
	public void log(String line, AttributeSet attributes) {
		...
		try {
			...
			document.insertString(offset, line,
					(attributes != null && colorEnabled) ? attributes
							: defaultAttributes);
	...
	/**
	 * Consume an input stream and print it to the dialog. The consumer will be
	...
	private void track(Process process) {
		final PrintWriter out = new PrintWriter(getOutputStream(Color.MAGENTA),
				true);
		Thread thread = new Thread(new Runnable() {
			public void run() {
				try {
	...
	private static String[] monospaceFontNames = { "Consolas",
			"DejaVu Sans Mono", "Bitstream Vera Sans Mono", "Lucida Console" };
