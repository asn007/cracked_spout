...
public class LegacyLoginFrame extends LoginFrame implements ActionListener,
		KeyListener {
	...
	private static final URL spoutcraftLogo = LegacyLoginFrame.class
			.getResource("/org/spoutcraft/launcher/resources/spoutcraft.png");
	private static final URL gearIcon = LegacyLoginFrame.class
			.getResource("/org/spoutcraft/launcher/resources/gear.png");
	...
	public LegacyLoginFrame() {
			...
			if (hasSavedPassword(user)) {
				...
				if (loginid == 1) {
					...
					ImageUtils.drawCharacter(contentPane, this,
							getSkinURL(user), 103, 170, loginSkin1Image);
					...
					ImageUtils.drawCharacter(contentPane, this,
							getSkinURL(user), 293, 170, loginSkin2Image);
		...
		JLabel build = new JLabel("Launcher Build: "
				+ (buildNumber == -1 ? "Custom" : "b" + buildNumber));
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getActionCommand().equals(USERNAME_ACTION)) {
			if (this.hasSavedPassword(usernameField.getSelectedItem()
					.toString())) {
				passwordField.setText(getSavedPassword(usernameField
		...
		// Username/Pass login
			...
			if (usernameField.getSelectedItem() != null) {
				...
				doLogin(usernameField.getSelectedItem().toString(), new String(
						passwordField.getPassword()), this);
				if (rememberCheckbox.isSelected()) {
					saveUsername(usernameField.getSelectedItem().toString(),
			...
			disableForm();
			doLogin(getAccountName(player1Name.getText()), this);
			// Login Skin 2
		} else if (e.getActionCommand().equals(LOGIN_SKIN_2_ACTION)) {
			disableForm();
			doLogin(getAccountName(player2Name.getText()), this);
			// Forget 1
	...
	private class ForgetThread extends Thread {
		...
		public void run() {
			while (!this.isInterrupted()) {
				if (forgetPlayer1.isVisible()
						&& !forgetPlayer1.getText().equals("Forget")) {
					if (forget1Time < System.currentTimeMillis()) {
						forgetPlayer1.setText("Forget");
					}
				}
				if (forgetPlayer2.isVisible()
						&& !forgetPlayer2.getText().equals("Forget")) {
